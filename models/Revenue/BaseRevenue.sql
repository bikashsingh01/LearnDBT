...
materialized='table'
    ...
    select 
			...
			--	left join "UDP_TRANSFORM_DEV"."DMR"."FNSPLITINCOLUMNSACTUAL"  fnSplit on fnSplit.GeneralLedger=stg.AccountDisplayValue
				left join ({{fnSplitInColumns('"UDP_STAGE_DEV"."DATASET_1011"."generaljournalaccountentryglobalstaging"','AccountDisplayValue')}})  fnSplit on fnSplit.AccountDisplayValue=stg.AccountDisplayValue
			...
			--	left join "UDP_TRANSFORM_DEV"."DMR"."FNSPLITINCOLUMNSBUDGET"  fnSplit on fnSplit.GeneralLedger=stg.DIMENSIONDISPLAYVALUE
				left join ({{fnSplitInColumns('"UDP_STAGE_DEV"."DATASET_1011"."amcglobalbudgetregisterentrystaging"','DIMENSIONDISPLAYVALUE')}})  fnSplit on fnSplit.DIMENSIONDISPLAYVALUE=stg.DIMENSIONDISPLAYVALUE
				left join "UDP_STAGE_DEV"."DATASET_1011"."MAINACCOUNTSTAGING" MA on MA.MAINACCOUNTID = fnSplit.Column1 
